// Điều phối Nghe & Đoán: Ôn Tập Tổng Hợp — trộn câu hỏi từ CẢ 9 game Nghe &
// Đoán (không dạy từ mới, chỉ ôn luyện). Máy đọc 1 từ hoặc câu tiếng Anh ngắn
// (giọng en-US thật, câu dài đọc chậm hơn), bé chạm đúng hình trong 4 lựa chọn
// — lọc được theo TỪNG GAME GỐC (9 lựa chọn) hoặc trộn tất cả ("Tất cả").

import {
  TOPICS, makeGame, currentRound, chooseOption, promptFor, rateFor,
} from './ontap.js';
import { speak, speakSequence, bindMute } from '../../to-mau/src/speech.js';
import { recordMiss, recordHit, missCount } from './misses.js';
...
function buildFilterRow() {
  ...
  for (const topic of TOPICS) {
    // Chip "Ôn chỗ yếu" hiện luôn số từ đang cần ôn để bé (và bố mẹ) thấy tiến bộ.
...
sayInstruction(t('ontap.help', 'Đây là màn ôn tập trộn toàn bộ từ vựng của cả 9 game Nghe & Đoán! Máy đọc một từ hoặc câu tiếng Anh — bé nghe thật kỹ rồi chạm vào hình đúng trong 4 hình bên dưới nhé. Nếu chọn sai, máy sẽ gợi ý để bé chọn lại một lần nữa. Chọn đúng liên tiếp 3 lần sẽ được điểm thưởng. Hàng nút trên cùng cho bé chọn ôn theo từng game hoặc trộn tất cả.'));
startRound();

// Hook cho e2e test
window.__ontap = { state, startRound };
