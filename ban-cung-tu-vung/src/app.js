// Điều phối Bắn Cung Từ Vựng — "mượn khung" ĐÚNG NGHĨA ĐEN: toàn bộ luật chơi
// (ngân hàng từ gộp cả 9 game + chủ đề "🎯 Ôn chỗ yếu", trộn từ đơn/câu, luật
// chọn-lại sai lần 1 được gợi ý, thưởng sao/quà) dùng lại NGUYÊN VẸN module
// đã kiểm thử kỹ của Ôn Tập Tổng Hợp (`nghe-doan-on-tap/src/ontap.js`) — file
// này CHỈ thêm 1 lớp da mới: bia hình tròn + cây cung + mũi tên bay khi bé
// chạm chọn, thay vì lưới nút chạm phẳng. Không có logic mới nào cần test
// riêng vì không có gì mới được viết ra ngoài phần trình chiếu (view).
...
function buildCorrectSequence(round) {
  ...
  return [
    { text: 'Bắn trúng rồi, bé giỏi quá!', lang: 'vi-VN', rate: 0.92 },
...
/**
 * Bắn trật lần ĐẦU (được chọn lại): gợi ý — đọc từ đúng (giọng Anh thật) +
 * nghĩa tiếng Việt rồi mời bé bắn lại. Chưa lộ đáp án trên màn hình.
 */
...
/** Bắn trật lần 2: lộ đáp án, đọc giải thích đầy đủ (từ + nghĩa + câu ví dụ) rồi qua bia mới. */
...
/* ===== Vẽ vòng chơi: hàng bia thay cho lưới nút phẳng ===== */
...
/* ===== Hiệu ứng bắn cung: mũi tên bay từ cây cung tới bia bé vừa chạm ===== */

function shootArrowTo(targetBtn, done) {
  const fieldRect = els.field.getBoundingClientRect();
  const bowRect = els.bow.getBoundingClientRect();
  const tgtRect = targetBtn.getBoundingClientRect();
  const startX = bowRect.left + bowRect.width / 2 - fieldRect.left;
  const startY = bowRect.top + bowRect.height / 2 - fieldRect.top;
  ...
  els.field.appendChild(arrow);

  requestAnimationFrame(() => {
    arrow.style.transition = 'left .3s cubic-bezier(.25,.6,.3,1), top .3s cubic-bezier(.25,.6,.3,1)';
  ...
  setTimeout(() => {
    ...
    els.bow.classList.remove('shoot');
    done();
  }, 320);
}
...
function onPick(opt, btn) {
  ...
  shootArrowTo(btn, () => {
    ...
    if (ev.retry) {
      // Bắn trật lần ĐẦU: chưa lộ đáp án — làm mờ bia vừa bắn, đọc gợi ý (từ +
      // nghĩa tiếng Việt) rồi mở khóa cho bé bắn lại đúng 1 lần.
...
function endRound() {
  ...
  if (g.won) {
    ...
    els.ovText.textContent = `${t('bancung.win', 'Cung thủ giỏi quá, bắn trúng rất nhiều bia!')}\n⭐ ${g.score} · 🔥 ${g.bestStreak}`;
    els.btnPlay.textContent = t('xepchu.next', 'MÀN TIẾP ▶');
    speak(t('bancung.win', 'Cung thủ giỏi quá, bắn trúng rất nhiều bia!'));
  } else {
    sfx.gameOver();
    els.ovEmoji.textContent = '💪';
...
sayInstruction(t('bancung.help', 'Đây là trò Bắn Cung Từ Vựng! Máy đọc một từ hoặc câu tiếng Anh — bé nghe thật kỹ rồi chạm vào đúng bia có hình phù hợp để bắn cung nhé. Nếu bắn trật, máy sẽ gợi ý để bé bắn lại một lần nữa. Bắn trúng liên tiếp 3 lần sẽ được điểm thưởng. Hàng nút trên cùng cho bé chọn ôn theo từng game hoặc trộn tất cả.'));
startRound();

// Hook cho e2e test
window.__bancung = { state, startRound };
