// Điều phối Câu Cá Từ Vựng — cùng công thức "mượn khung" như Bắn Cung/Ném
// Lon Từ Vựng: toàn bộ luật chơi (ngân hàng từ gộp cả 9 game + chủ đề "🎯 Ôn
// chỗ yếu", trộn từ đơn/câu, luật chọn-lại, thưởng sao/quà) dùng lại NGUYÊN
// VẸN module đã kiểm thử kỹ của Ôn Tập Tổng Hợp (`nghe-doan-on-tap/src/ontap.js`)
// — file này chỉ thêm 1 lớp da mới: ao cá bơi lững lờ + cần câu trên đỉnh,
// lưỡi câu thả xuống đúng con cá bé chạm, câu trúng thì cá bay lên khỏi mặt
// nước. Không có logic mới nào cần test riêng vì không viết thêm gì ngoài
// phần trình chiếu (view).
...
function buildCorrectSequence(round) {
  ...
  return [
    { text: 'Câu trúng rồi, bé giỏi quá!', lang: 'vi-VN', rate: 0.92 },
...
/**
 * Câu trật lần ĐẦU (được chọn lại): gợi ý — đọc từ đúng (giọng Anh thật) +
 * nghĩa tiếng Việt rồi mời bé câu lại. Chưa lộ đáp án trên màn hình.
 */
function buildHintSequence(round) {
  ...
  return [
    ...
    { text: word, lang: 'en-US', rate: rateFor('word') },
    { text: `là ${vi}. Bé hãy câu lại nhé!`, lang: 'vi-VN', rate: 0.88 },
  ];
}

/** Câu trật lần 2: lộ đáp án, đọc giải thích đầy đủ (từ + nghĩa + câu ví dụ) rồi qua cá mới. */
function buildRevealSequence(round) {
  ...
  return [
    ...
    { text: sentence, lang: 'en-US', rate: rateFor('sentence') },
    { text: `nghĩa là ${sentenceVi}. Lần sau bé sẽ câu trúng!`, lang: 'vi-VN', rate: 0.88 },
  ];
}
...
/* ===== Vẽ vòng chơi: ao cá bơi thay cho lưới nút phẳng ===== */
...
/* ===== Hiệu ứng câu cá: lưỡi câu thả từ cần câu xuống đúng cá bé vừa chạm ===== */

function castHookTo(targetBtn, done) {
  const fieldRect = els.field.getBoundingClientRect();
  const rodRect = els.rod.getBoundingClientRect();
  const tgtRect = targetBtn.getBoundingClientRect();
  const startX = rodRect.left + rodRect.width / 2 - fieldRect.left;
  const startY = rodRect.top + rodRect.height / 2 - fieldRect.top;
  const endX = tgtRect.left + tgtRect.width / 2 - fieldRect.left;
  const endY = tgtRect.top + tgtRect.height / 2 - fieldRect.top;

  els.rod.classList.add('cast');
  const hook = document.createElement('div');
  hook.className = 'hook';
  hook.textContent = '🪝';
  hook.style.left = `${startX}px`;
  hook.style.top = `${startY}px`;
  hook.style.transform = 'translate(-50%, -50%) rotate(-12deg)';
  els.field.appendChild(hook);

  // Chặng 1: lưỡi câu đu ngang tới đúng phía trên con cá (nhanh, hơi lắc lư
  // như dây câu thật) — rồi mới THẢ THẲNG XUỐNG theo trọng lực ở chặng 2,
  // thay vì bay chéo 1 mạch trông giả tạo như trước.
  requestAnimationFrame(() => {
    hook.style.transition = 'left .16s ease-out, transform .16s ease-out';
    hook.style.left = `${endX}px`;
    hook.style.transform = 'translate(-50%, -50%) rotate(10deg)';
  });
  setTimeout(() => {
    hook.style.transition = 'top .22s cubic-bezier(.55,0,1,.45), transform .22s ease-in';
    hook.style.top = `${endY}px`;
    hook.style.transform = 'translate(-50%, -50%) rotate(0deg)';
  }, 160);
  setTimeout(() => {
    hook.remove();
    spawnImpact(endX, endY);
    els.rod.classList.remove('cast');
    done();
  }, 390);
}
...
function onPick(opt, btn) {
  ...
  castHookTo(btn, () => {
    ...
    if (ev.retry) {
      // Câu trật lần ĐẦU: chưa lộ đáp án — làm mờ cá vừa câu, đọc gợi ý (từ +
      // nghĩa tiếng Việt) rồi mở khóa cho bé câu lại đúng 1 lần.
...
function endRound() {
  ...
  if (g.won) {
    ...
    els.ovText.textContent = `${t('cauca.win', 'Bé câu giỏi quá, câu trúng rất nhiều cá!')}\n⭐ ${g.score} · 🔥 ${g.bestStreak}`;
    els.btnPlay.textContent = t('xepchu.next', 'MÀN TIẾP ▶');
    speak(t('cauca.win', 'Bé câu giỏi quá, câu trúng rất nhiều cá!'));
  } else {
    sfx.gameOver();
    els.ovEmoji.textContent = '💪';
...
sayInstruction(t('cauca.help', 'Đây là trò Câu Cá Từ Vựng! Máy đọc một từ hoặc câu tiếng Anh — bé nghe thật kỹ rồi chạm vào đúng con cá có hình phù hợp để câu nhé. Nếu câu trật, máy sẽ gợi ý để bé câu lại một lần nữa. Câu trúng liên tiếp 3 lần sẽ được điểm thưởng. Hàng nút trên cùng cho bé chọn ôn theo từng game hoặc trộn tất cả.'));
startRound();

// Hook cho e2e test
window.__cauca = { state, startRound };
