...
check('WORD_BANK has at least 65 entries', () => {
  assert.ok(WORD_BANK.length >= 65, `only ${WORD_BANK.length} entries`);
});
...
check('position entries (7 gioi tu) are wired with a local images/ SVG', () => {
  const withSvg = ['in', 'on', 'under', 'behind', 'infrontof', 'nextto', 'between'];
  for (const id of withSvg) {
    const w = WORD_BANK.find((e) => e.id === id);
    assert.ok(w, `expected entry ${id} to exist`);
    assert.ok(w.img && w.img.startsWith('images/pos-'), `expected entry ${id} to use a pos-*.svg`);
  }
});
...
check('promptFor: reads the plain word in "word" mode, the full sentence in "sentence" mode', () => {
  const target = WORD_BANK.find((w) => w.id === 'run');
  assert.equal(promptFor({ target, mode: 'word' }), 'run');
  assert.equal(promptFor({ target, mode: 'sentence' }), 'I run in the park.');
});

check('pickRound: target is included among options, options has no duplicate ids', () => {
  const rng = seeded(7);
  const pool = wordsForTopic('toy');
...
check('pickRound: avoids already-used target ids when possible', () => {
  const rng = seeded(3);
  const pool = wordsForTopic('place');
...
check('makeGame(topic, ...): every round option belongs to that topic pool', () => {
  const g = makeGame('action', 1, seeded(5));
  const pool = new Set(wordsForTopic('action').map((w) => w.id));
...
check('chooseOption: correct answer increases score and streak', () => {
  const g = makeGame('hobby', 0, seeded(2));
...
check('chooseOption: game ends after all rounds, over+won set correctly', () => {
  const g = makeGame('place', 0, seeded(4));
...
check('chooseOption: no-op once game is over', () => {
  const g = makeGame('toy', 0, seeded(1));
  while (!g.over) {
    const r = currentRound(g);
    chooseOption(g, r.target.id);
  }
