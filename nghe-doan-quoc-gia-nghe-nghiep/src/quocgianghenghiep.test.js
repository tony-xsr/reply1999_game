...
check('WORD_BANK has at least 70 entries', () => {
  assert.ok(WORD_BANK.length >= 70, `only ${WORD_BANK.length} entries`);
});
...
check('promptFor: reads the plain word in "word" mode, the full sentence in "sentence" mode', () => {
  const target = WORD_BANK.find((w) => w.id === 'vietnam');
  assert.equal(promptFor({ target, mode: 'word' }), 'Vietnam');
  assert.equal(promptFor({ target, mode: 'sentence' }), 'I live in Vietnam.');
});

check('pickRound: target is included among options, options has no duplicate ids', () => {
  const rng = seeded(7);
  const pool = wordsForTopic('country');
...
check('makeGame(topic, ...): every round option belongs to that topic pool', () => {
  const g = makeGame('number', 1, seeded(5));
  const pool = new Set(wordsForTopic('number').map((w) => w.id));
...
check('chooseOption: correct answer increases score and streak', () => {
  const g = makeGame('tool', 0, seeded(2));
...
check('chooseOption: game ends after all rounds, over+won set correctly', () => {
  const g = makeGame('subject', 0, seeded(4));
...
check('chooseOption: no-op once game is over', () => {
  const g = makeGame('country', 0, seeded(1));
  while (!g.over) {
    const r = currentRound(g);
    chooseOption(g, r.target.id);
  }
