...
check('real-photo entries (black hole, 4 loai chim) are wired with a local images/ path', () => {
  const withPhoto = ['blackhole', 'pelican', 'woodpecker', 'hummingbird', 'kingfisher', 'seagull', 'crow'];
...
check('promptFor: reads the plain word in "word" mode, the full sentence in "sentence" mode', () => {
  const target = WORD_BANK.find((w) => w.id === 'lion');
  assert.equal(promptFor({ target, mode: 'word' }), 'lion');
  assert.equal(promptFor({ target, mode: 'sentence' }), 'The lion is the king of the jungle.');
});

check('pickRound: target is included among options, options has no duplicate ids', () => {
  const rng = seeded(7);
  const pool = wordsForTopic('wildlife');
...
check('makeGame(topic, ...): every round option belongs to that topic pool', () => {
  const g = makeGame('marine', 1, seeded(5));
  const pool = new Set(wordsForTopic('marine').map((w) => w.id));
...
check('chooseOption: correct answer increases score and streak', () => {
  const g = makeGame('wildlife', 0, seeded(2));
...
check('chooseOption: game ends after all rounds, over+won set correctly', () => {
  const g = makeGame('sky', 0, seeded(4));
...
check('chooseOption: no-op once game is over', () => {
  const g = makeGame('bird', 0, seeded(1));
  while (!g.over) {
    const r = currentRound(g);
    chooseOption(g, r.target.id);
  }
