...
check('WORD_BANK has at least 80 entries', () => {
  assert.ok(WORD_BANK.length >= 80, `only ${WORD_BANK.length} entries`);
});
...
check('real-photo/SVG entries (calculator, fisherman, barber, scoreboard, 5 cay gia dinh) are wired with a local images/ path', () => {
  const withPhoto = ['calculator', 'fisherman', 'barber', 'scoreboard', 'parents', 'grandparents', 'aunt', 'uncle', 'cousin'];
...
check('promptFor: reads the plain word in "word" mode, the full sentence in "sentence" mode', () => {
  const target = WORD_BANK.find((w) => w.id === 'teacher');
  assert.equal(promptFor({ target, mode: 'word' }), 'teacher');
  assert.equal(promptFor({ target, mode: 'sentence' }), 'The teacher teaches math.');
});

check('pickRound: target is included among options, options has no duplicate ids', () => {
  const rng = seeded(7);
  const pool = wordsForTopic('sport');
...
check('makeGame(topic, ...): every round option belongs to that topic pool', () => {
  const g = makeGame('school', 1, seeded(5));
  const pool = new Set(wordsForTopic('school').map((w) => w.id));
...
check('chooseOption: correct answer increases score and streak', () => {
  const g = makeGame('family', 0, seeded(2));
...
check('chooseOption: game ends after all rounds, over+won set correctly', () => {
  const g = makeGame('compete', 0, seeded(4));
...
check('chooseOption: no-op once game is over', () => {
  const g = makeGame('job', 0, seeded(1));
  while (!g.over) {
    const r = currentRound(g);
    chooseOption(g, r.target.id);
  }
