...
check('WORD_BANK has at least 85 entries', () => {
  assert.ok(WORD_BANK.length >= 85, `only ${WORD_BANK.length} entries`);
});
...
check('body-diagram entries (7 khop/bo phan) are wired with a local images/ SVG', () => {
  const withSvg = ['knee', 'elbow', 'shoulder', 'ankle', 'wrist', 'neck', 'cheek'];
  for (const id of withSvg) {
    const w = WORD_BANK.find((e) => e.id === id);
    assert.ok(w, `expected entry ${id} to exist`);
    assert.ok(w.img && w.img.startsWith('images/body-'), `expected entry ${id} to use a body-*.svg`);
  }
});
...
check('promptFor: reads the plain word in "word" mode, the full sentence in "sentence" mode', () => {
  const target = WORD_BANK.find((w) => w.id === 'tshirt');
  assert.equal(promptFor({ target, mode: 'word' }), 'T-shirt');
  assert.equal(promptFor({ target, mode: 'sentence' }), 'I wear a T-shirt.');
});

check('pickRound: target is included among options, options has no duplicate ids', () => {
  const rng = seeded(7);
  const pool = wordsForTopic('household');
...
check('makeGame(topic, ...): every round option belongs to that topic pool', () => {
  const g = makeGame('tech', 1, seeded(5));
  const pool = new Set(wordsForTopic('tech').map((w) => w.id));
...
check('chooseOption: correct answer increases score and streak', () => {
  const g = makeGame('clothes', 0, seeded(2));
...
check('chooseOption: game ends after all rounds, over+won set correctly', () => {
  const g = makeGame('activity', 0, seeded(4));
...
check('chooseOption: no-op once game is over', () => {
  const g = makeGame('household', 0, seeded(1));
  while (!g.over) {
    const r = currentRound(g);
    chooseOption(g, r.target.id);
  }
