// Điều phối Đá Phạt Đền Từ Vựng — "mượn khung" y hệt các game Ôn Tập Vui
...
// mới: khung thành có lưới + thủ môn phản ứng (bay SAI hướng khi bé sút trúng,
// lao tới ĐÚNG hướng khi bé sút trật) + cầu thủ sút bóng ở đáy.
...
const els = {
  ...
  field: $('field'), kicker: $('kicker'), keeper: $('keeper'),
};
...
function buildCorrectSequence(round) {
  ...
  return [
    { text: 'Vào rồi, bé sút giỏi quá!', lang: 'vi-VN', rate: 0.92 },
...
function buildHintSequence(round) {
  ...
  return [
    { text: 'Thủ môn cản được rồi.', lang: 'vi-VN', rate: 0.92 },
    { text: word, lang: 'en-US', rate: rateFor('word') },
    { text: `là ${vi}. Bé hãy sút lại nhé!`, lang: 'vi-VN', rate: 0.88 },
  ];
}
...
function buildRevealSequence(round) {
  const { word, vi, sentence, sentenceVi } = round.target;
  return [
    { text: 'Vẫn bị cản. Đáp án là', lang: 'vi-VN', rate: 0.92 },
    { text: word, lang: 'en-US', rate: rateFor('word') },
    { text: `nghĩa là ${vi}.`, lang: 'vi-VN', rate: 0.88 },
    { text: sentence, lang: 'en-US', rate: rateFor('sentence') },
    { text: `nghĩa là ${sentenceVi}. Lần sau bé sẽ sút vào!`, lang: 'vi-VN', rate: 0.88 },
  ];
}
...
/* ===== Vẽ vòng chơi: khung thành nhiều góc thay cho lưới nút phẳng ===== */
...
/* ===== Hiệu ứng sút bóng: bóng bay từ cầu thủ dưới đáy tới đúng góc bé chạm ===== */

function kickBallTo(targetBtn, done) {
  const fieldRect = els.field.getBoundingClientRect();
  const kickerRect = els.kicker.getBoundingClientRect();
  const tgtRect = targetBtn.getBoundingClientRect();
  const startX = kickerRect.left + kickerRect.width / 2 - fieldRect.left;
  const startY = kickerRect.top + kickerRect.height / 2 - fieldRect.top;
  const endX = tgtRect.left + tgtRect.width / 2 - fieldRect.left;
  const endY = tgtRect.top + tgtRect.height / 2 - fieldRect.top;

  els.kicker.classList.add('kick');
  ...
  requestAnimationFrame(() => {
    // Bong xoay tron trong luc bay — giong 1 cu sut that thay vi truot phang.
    ball.style.transition = 'left .24s ease-in, top .24s ease-in, transform .24s linear';
    ball.style.left = `${endX}px`;
    ball.style.top = `${endY}px`;
    ball.style.transform = 'translate(-50%, -50%) rotate(420deg)';
  });
  setTimeout(() => {
    ball.remove();
    spawnImpact(endX, endY);
    els.kicker.classList.remove('kick');
    done(endX);
  }, 260);
}

/** Thủ môn phản ứng: đá TRÚNG -> bay SAI hướng (không cản được); đá TRẬT ->
 * lao đúng tới điểm bóng để cản. */
function reactKeeper(ballX, saved) {
  const fieldRect = els.field.getBoundingClientRect();
  const keeperCenter = fieldRect.width / 2;
  if (saved) {
    const offset = Math.max(-90, Math.min(90, ballX - keeperCenter));
    els.keeper.style.transform = `translateX(calc(-50% + ${offset}px))`;
  } else {
    const wrongOffset = ballX > keeperCenter ? -90 : 90;
    els.keeper.style.transform = `translateX(calc(-50% + ${wrongOffset}px))`;
  }
}
...
function onPick(opt, btn) {
  ...
  kickBallTo(btn, (ballX) => {
    const round = currentRound(g);
    const ev = chooseOption(g, opt.id);

    if (ev.retry) {
      // Sút trật lần ĐẦU (thủ môn cản được): chưa lộ đáp án — đọc gợi ý (từ +
      // nghĩa tiếng Việt) rồi mở khóa cho bé sút lại đúng 1 lần.
      reactKeeper(ballX, true);
...
function endRound() {
  ...
  if (g.won) {
    ...
    els.ovText.textContent = `${t('daphat.win', 'Chân sút cừ khôi, ghi rất nhiều bàn thắng!')}\n⭐ ${g.score} · 🔥 ${g.bestStreak}`;
    els.btnPlay.textContent = t('xepchu.next', 'MÀN TIẾP ▶');
    speak(t('daphat.win', 'Chân sút cừ khôi, ghi rất nhiều bàn thắng!'));
  } else {
    sfx.gameOver();
    els.ovEmoji.textContent = '💪';
...
sayInstruction(t('daphat.help', 'Đây là trò Đá Phạt Đền Từ Vựng! Máy đọc một từ hoặc câu tiếng Anh — bé nghe thật kỹ rồi chạm vào đúng góc khung thành có hình phù hợp để sút nhé. Nếu bị thủ môn cản, máy sẽ gợi ý để bé sút lại một lần nữa. Sút vào liên tiếp 3 lần sẽ được điểm thưởng. Hàng nút trên cùng cho bé chọn ôn theo từng game hoặc trộn tất cả.'));
startRound();

// Hook cho e2e test
window.__daphat = { state, startRound };
