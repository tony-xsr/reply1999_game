// Điều phối Bắt Chuột Chũi Từ Vựng — "mượn khung" y hệt Bắn Cung/Ném Lon/Câu Cá:
// toàn bộ luật chơi (ngân hàng từ, chủ đề "🎯 Ôn chỗ yếu", luật chọn-lại, thưởng
// sao/quà) dùng lại NGUYÊN VẸN module đã kiểm thử kỹ của Ôn Tập Tổng Hợp
// (`nghe-doan-on-tap/src/ontap.js`) — file này CHỈ thêm 1 lớp da mới: lỗ đất có
// chuột chũi nhô lên + búa đập từ trên xuống khi bé chạm chọn.
...
function buildCorrectSequence(round) {
  ...
  return [
    { text: 'Đập trúng rồi, bé giỏi quá!', lang: 'vi-VN', rate: 0.92 },
...
function buildHintSequence(round) {
  ...
  return [
    ...
    { text: word, lang: 'en-US', rate: rateFor('word') },
    { text: `là ${vi}. Bé hãy đập lại nhé!`, lang: 'vi-VN', rate: 0.88 },
  ];
}
...
function buildRevealSequence(round) {
  ...
  return [
    ...
    { text: sentence, lang: 'en-US', rate: rateFor('sentence') },
    { text: `nghĩa là ${sentenceVi}. Lần sau bé sẽ đập trúng!`, lang: 'vi-VN', rate: 0.88 },
  ];
}
...
/* ===== Vẽ vòng chơi: hàng lỗ chuột chũi thay cho lưới nút phẳng ===== */
...
/* ===== Hiệu ứng đập búa: búa bay từ trên xuống đúng lỗ bé vừa chạm ===== */

function smashMoleAt(targetBtn, done) {
  ...
  els.field.appendChild(hammer);

  requestAnimationFrame(() => {
    hammer.style.transition = 'top .2s cubic-bezier(.4,0,1,1), transform .2s ease-in';
    hammer.style.top = `${endY}px`;
    hammer.style.transform = 'translate(-50%, -50%) rotate(15deg)';
  });
  setTimeout(() => {
    hammer.remove();
...
function onPick(opt, btn) {
  ...
  smashMoleAt(btn, () => {
    ...
    if (ev.retry) {
      // Đập trật lần ĐẦU: chưa lộ đáp án — chuột chũi vừa đập chui xuống lỗ,
      // đọc gợi ý (từ + nghĩa tiếng Việt) rồi mở khóa cho bé đập lại đúng 1 lần.
...
function endRound() {
  ...
  if (g.won) {
    ...
    els.ovText.textContent = `${t('batchuot.win', 'Thợ săn chuột giỏi quá, đập trúng rất nhiều chuột chũi!')}\n⭐ ${g.score} · 🔥 ${g.bestStreak}`;
    els.btnPlay.textContent = t('xepchu.next', 'MÀN TIẾP ▶');
    speak(t('batchuot.win', 'Thợ săn chuột giỏi quá, đập trúng rất nhiều chuột chũi!'));
  } else {
    sfx.gameOver();
    els.ovEmoji.textContent = '💪';
...
sayInstruction(t('batchuot.help', 'Đây là trò Bắt Chuột Chũi Từ Vựng! Máy đọc một từ hoặc câu tiếng Anh — bé nghe thật kỹ rồi chạm vào đúng lỗ có chú chuột chũi mang hình phù hợp để đập nhé. Nếu đập trật, máy sẽ gợi ý để bé đập lại một lần nữa. Đập trúng liên tiếp 3 lần sẽ được điểm thưởng. Hàng nút trên cùng cho bé chọn ôn theo từng game hoặc trộn tất cả.'));
startRound();

// Hook cho e2e test
window.__batchuot = { state, startRound };
